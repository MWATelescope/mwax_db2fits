...
try:
    ...
    while True:
        ...
        print("{0}: Status: {1} Readers: {2} Buffs tot: {3} {4:.1f}MB avail: {5} {6:.1f}MB clear: {7} full: {8} read: {9} {10:.1f} MB".format(
            datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), sint32_status, sint32_n_readers,
            sint32_data_nbufs, (uint64_data_bytes / MEGABYTE),
            uint64_data_available_bufs, (uint64_data_available_bufs*uint64_data_bufsz/MEGABYTE),
            uint64_data_clear_bufs,
            uint64_data_full_bufs,
            uint64_data_bufs_read, (uint64_data_bufs_read*uint64_data_bufsz/MEGABYTE)))
        sys.stdout.flush()
finally:
