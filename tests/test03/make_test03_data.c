...
void usage()
{
    printf("make_test03_data subobs_number header output_file\n"
...
int main(int argc, char **argv)
{
    ...
    {
        // Write visibilities
        if (write_visibilities_hdu(output_file, NBASELINES, NFINECHAN, NPOLS, NVALUES, timestep, (((subobs_number - 1) * NTIMESTEPS) + timestep) * 102) != EXIT_SUCCESS)
        {
            exit(EXIT_FAILURE);
        }

        // Write weights
        if (write_weights_hdu(output_file, NBASELINES, NFINECHAN, NPOLS, NVALUES, timestep, (((subobs_number - 1) * NTIMESTEPS) + (timestep - 1)) * 0.06, 0.06) != EXIT_SUCCESS)
        {
            exit(EXIT_FAILURE);
        }
    }
